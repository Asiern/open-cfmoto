...
const envUserAgent =
  ...
  (globalThis as { process?: { env?: Record<string, string | undefined> } }).process?.env
    ?.CLOUD_USER_AGENT;
const envDiscoveryBaseUrl =
  (globalThis as { process?: { env?: Record<string, string | undefined> } }).process?.env
    ?.EXPO_PUBLIC_CLOUD_DISCOVERY_BASE_URL ??
  (globalThis as { process?: { env?: Record<string, string | undefined> } }).process?.env
    ?.CLOUD_DISCOVERY_BASE_URL;
...
export const CLOUD_CONFIG = {
  ...
  DISCOVERY_BASE_URL:
    (envDiscoveryBaseUrl?.trim().replace(/\/$/, '') || 'https://tapi.cfmoto-oversea.com/v1.0'),
  ENDPOINTS: {
    ...
    LOGIN_AREA_LIST: '/fuel-user/serveruser/app/auth/user/getLoginArea',
